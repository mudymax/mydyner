...
- 🎯 Entry points to execute your program using `python -m <mydyner>` or `$ mydyner` with basic CLI argument parsing.
...
---
# mydyner Flask Application

Awesome mydyner created by mudymax
...
```bash
git clone https://github.com/mudymax/mydyner mydyner
cd mydyner
make install
```
...
```bash
mydyner create-db   # run once
mydyner populate-db  # run once (optional)
mydyner add-user -u admin -p 1234  # ads a user
mydyner run
```
