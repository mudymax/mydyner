...
original_author="mudymax"
original_name="mydyner"
original_urlname="mydyner"
original_description="Awesome mydyner created by mudymax" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/mydyner "${name}"
...
then
    ...
    echo "then run 'mydyner' or 'python -m mydyner'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'mydyner' or 'python -m mydyner' or 'poetry run mydyner'"
fi
